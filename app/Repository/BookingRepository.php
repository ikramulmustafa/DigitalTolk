    ...
    public function bookingExpireNoAccepted(Request $request)
    {
        ...
        if ($cuser && ($cuser->is('superadmin') || $cuser->is('admin'))) {
            ...
            if ($request->filled('lang')) {
                $allJobs->whereIn('jobs.from_language_id', $request->input('lang'));
            }

            if ($request->filled('customer_email')) {
                $allJobs->whereIn('jobs.user_id', $request->input('customer_email'));
            }

            if ($request->filled('translator_email')) {
                $allJobs->whereIn('jobs.translator_id', $request->input('translator_email'));
            }

            if ($request->filled('from') && $request->filled('to')) {
                $allJobs->whereBetween('jobs.created_at', [$request->input('from'), $request->input('to')]);
            }

            if ($request->filled('job_type')) {
                $allJobs->whereIn('jobs.job_type', $request->input('job_type'));
            }
